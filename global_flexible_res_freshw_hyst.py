...
class GlobalFlexibleResolutionSetup(VerosSetup):
    ...
    def _get_data(self, vs, var, idx=None):
        ...
        with h5netcdf.File('forcing_1deg_global_interp_hosing.nc', 'r', **kwargs) as forcing_file:
    ...
    def set_initial_conditions(self, vs):
        ...
        ### HERE read freshwater mask on fine grid. Then interpolate
    ...
    def set_forcing(self, vs):
        ...
        ampl = 0.00000006#0.0000006#0.000004#0.0000002
        t_run = 300.
        t_ramp = 200. #ramping time in years
        t_start = 4000. #simulation time in years of spin-up
        t_sim = vs.time/360./86400.
        t_diff = (t_sim-t_start)%(t_run)
        count = math.floor((t_sim-t_start)/t_run)
        if count>11:
            count = 24 - count
            forc_val = ampl*count - ampl*t_diff/t_ramp*(1. - np.heaviside(t_diff/t_ramp -1.,0)) - ampl*np.heaviside(t_diff/t_ramp -1.,0) 
    ...
    def set_diagnostics(self, vs):
        ...
        vs.diagnostics['tracer_monitor'].output_frequency = 360 * 86400.
        vs.diagnostics['snapshot'].output_frequency = 5 * 360* 86400.
        vs.diagnostics['overturning'].output_frequency = 5 * 360 * 86400
        vs.diagnostics['overturning'].sampling_frequency = 5 * 360 * 86400.
        vs.diagnostics['energy'].output_frequency = 5 * 360 * 86400
        vs.diagnostics['energy'].sampling_frequency = 5 * 360 * 86400.
        vs.diagnostics['averages'].output_frequency = 5 * 360 * 86400 #10 * 86400
        vs.diagnostics['averages'].sampling_frequency = 86400#3600.
